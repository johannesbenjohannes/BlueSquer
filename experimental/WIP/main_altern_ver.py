...
class Dash(Ability):
    ...
    def activate(self):
        ...
        self.activation_timestamp = time()
        self.timer = 0
        self.active = True
    ...
    def canActivate(self):
        ...
        return time() - self.activation_timestamp > self.cooldown
...
startMenu = UI.Panel.fromLayer(ui_layer)
startMenu.build({
    "frame": {
        "name": "title_frame",
        "position": (.5,0,.5,0),
...
characterMenu = UI.Panel.fromLayer(ui_layer)
characterMenu.build({
    "frame": {
        "name": "main_frame",
        "anchorPoint": (.5, .5),
...
upgradeMenu = UI.Panel.fromLayer(ui_layer)
upgradeMenu.build({
    "frame": {
        "name": "main_frame",
        "anchorPoint": (.5, .5),
...
def loadMenu(name: str):
    global current_menu_name, current_menu
...
startMenu.descendants["play_button"].bindAction("clicked", lambda: loadMenu("Character"))
characterMenu.descendants["main_button"].bindAction("clicked", lambda: loadMenu("Upgrade"))
upgradeMenu.descendants["main_button"].bindAction("clicked", lambda: loadMenu("Start"))

def main():
    def draw_squer(window, at):
        pygame.draw.rect(window, LIGHT_BLUE, (*at.components, 10, 10))
